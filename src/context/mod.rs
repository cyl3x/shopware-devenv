...
impl Context {
    pub fn new(config: &Config, origin_path: &Path) -> Option<Self> {
        let mut custom: Option<CustomContext> = None;

        let mut curr_dir = origin_path.to_owned();

        while !curr_dir.ends_with("/") {
            if let Some(custom_context) = CustomContext::new(config, &curr_dir) {
                custom = Some(custom_context);
            }

            if let Some(platform_context) = PlatformContext::new(config, &curr_dir) {
                return Some(Self {
